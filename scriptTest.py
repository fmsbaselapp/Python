...
schulhauslinksTest = ('https://display.edubs.ch/gl1',)
for haupturl in schulhauslinksTest:
    ...
    if 1 == 1:
        ...
        anlaessehtml = r.html.find('body > div > div')[
            anzahlausfaellekomplett1]
        ...
        for loeschkombo in tag:
            ...
            db.collection(u'Ausfaelle').document(u'Schulen').collection(
                schule).document(loeschtag).delete()
        for loeschkombo in tag:
            loeschtag = b + loeschkombo
            db.collection(u'Ausfaelle').document(u'Schulen').collection(
                schule).document(loeschtag).delete()
        db.collection(u'Anlaesse').document(u'Schulen').collection(
            schule).document(anlassname).delete()
        ...
        if keinausfalltag == False:
            db.collection(u'Ausfaelle').document(u'Schulen').collection(
                schule).document(ersterausfalltag1).set(leer)
        if zweiterausfall == True:
            db.collection(u'Ausfaelle').document(u'Schulen').collection(
                schule).document(zweiteausfalltag1).set(leer)
        if keinanlass == False:
            db.collection(u'Anlaesse').document(u'Schulen').collection(
                schule).document(anlassname).set(leer)
